...
/// Sorted General_Category range table. Enumerable at comptime, unlike the
/// per-code-point page tables, so consumers (e.g. a regex engine) can resolve
/// `\p{Lu}`/`\p{L}`/`\d` into code-point ranges. Generated by
/// `zig build generate-ranges`.
pub const category_ranges = @import("generated/category_ranges.zig");
/// Sorted DerivedCoreProperties range table (enumerable; see `category_ranges`).
pub const derived_ranges = @import("generated/derived_ranges.zig");
/// Sorted PropList range tables (White_Space, Join_Control).
pub const prop_list_ranges = @import("generated/prop_list_ranges.zig");
...
/// One entry of `category_runs`: an inclusive `[start, end]` run of equal
/// General_Category.
...
/// Returns whether `code_point` is a "word" character in the Unicode sense used
